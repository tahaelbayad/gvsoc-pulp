 ...
 * Note:
 *      Here we only support integer for matrix multiply
...
Redmule::Redmule(vp::ComponentConf &config)
    : vp::Component(config)
{
    //Initialize interface
    this->traces.new_trace("trace", &this->trace, vp::DEBUG);
    this->input_itf.set_req_meth(&Redmule::req);
    ...
    if (this->compute_able == 1)
    {
        ...
        this->y_buffer_preload  = 0;
        this->w_buffer          = 0;
        this->x_buffer          = 0;
        this->z_buffer_compute  = 0;
        this->z_buffer_previos  = 0;
    }
    ...
    this->trace.msg("[Redmule] Model Initialization Done!\n");
}

void Redmule::init_redmule_meta_data(){
...
uint32_t Redmule::next_iteration(){
    this->iter_k += 1;
...
uint32_t Redmule::calculate_tile_base_address(uint32_t base, uint32_t stride, uint32_t tile_col, uint32_t tile_row, uint32_t i, uint32_t j){
    /*
    *       ----
    *   col |  |
    *       ----
...
uint32_t Redmule::inc_addr(uint32_t addr, uint32_t stride, uint32_t tile_row){
    if (this->fold_tiles_mapping)
    {
...
uint32_t Redmule::next_addr(){
    ...
    if (this->y_acc_block > 0)
    {
        ...
        this->y_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Address] Y tile at 0x%11x | #Y tile left %d\n", addr, this->y_acc_block);
    ...
    } else if (this->w_acc_block > 0)
    {
        ...
        this->w_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Address] W tile at 0x%11x | #W tile left %d\n", addr, this->w_acc_block);
        this->tcdm_req->set_is_write(0);
        this->tcdm_req->set_size(buffer_w * this->elem_size);
    ...
    } else if (this->x_acc_block > 0)
    {
        ...
        this->x_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Address] X tile at 0x%11x | #X tile left %d\n", addr, this->x_acc_block);
        this->tcdm_req->set_is_write(0);
    ...
    } else if (this->z_acc_block > 0)
    {
        ...
        this->z_acc_block -= 1;
        this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Address] Z tile at 0x%11x | #Z tile left %d\n", addr, this->z_acc_block);
        this->tcdm_req->set_is_write(1);
        this->tcdm_req->set_size(this->z_store_width * this->elem_size);
        this->iter_instruction  = INSTR_STOR_Z;
    } else {
        this->trace.fatal("[Redmule][Address] INVALID redmule address iteration : No tiles to access\n");
    }

    return addr;
}

void Redmule::print_array(void * ptr, uint32_t length, uint32_t cut_line){
    ...
    this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][--Debug Array--]           \n%s \n",oss.str().c_str());
}

void Redmule::process_compute(){
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->elem_size;
...
void Redmule::process_iter_instruction(){
...
uint32_t Redmule::get_routine_access_block_number(){
...
uint32_t Redmule::get_preload_access_block_number(){
    uint32_t total_blocks       = 0;
...
uint32_t Redmule::get_storing_access_block_number(){
    uint32_t total_blocks       = 0;
    uint32_t buffer_h           = this->ce_height;
    uint32_t buffer_w           = this->ce_width * (this->ce_pipe + 1);
    uint32_t buffer_n           = this->bandwidth / this->elem_size;
...
uint32_t Redmule::get_routine_to_storing_latency(){
    return this->ce_width * (this->ce_pipe + 1);
    // return 0;
}

uint32_t Redmule::get_redmule_array_runtime(){
    uint32_t tcdms_bw       = this->bandwidth / this->elem_size;
...
vp::IoReqStatus Redmule::req(vp::Block *__this, vp::IoReq *req)
{
    ...
    // _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule] access (offset: 0x%x, size: 0x%x, is_write: %d, data:%x)\n", offset, size, is_write, *(uint32_t *)data);
    ...
    if ((is_write == 0) && (offset == 32) && (_this->redmule_query == NULL) && (_this->state.get() == IDLE))
    {
        ...
        ************************/
        //Sanity Check
        _this->trace.msg("[Redmule] redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
        if ((_this->m_size == 0)||(_this->n_size == 0)||(_this->k_size == 0))
        {
            _this->trace.fatal("[Redmule] INVALID redmule configuration (M-N-K): %d, %d, %d\n", _this->m_size, _this->n_size, _this->k_size);
            return vp::IO_REQ_OK;
        }
        ...
        *************************/
        ...
        switch (offset) {
            case 64:
                _this->x_addr = value;
                _this->trace.msg("[Redmule] Set X addr 0x%x)\n", value);
                break;
            case 68:
                _this->w_addr = value;
                _this->trace.msg("[Redmule] Set W addr 0x%x)\n", value);
                break;
            case 72:
                _this->z_addr = value;
                _this->y_addr = value;
                _this->trace.msg("[Redmule] Set Y addr 0x%x)\n", value);
                _this->trace.msg("[Redmule] Set Z addr 0x%x)\n", value);
                break;
            case 76:
                _this->m_size = value & 0xFFFF;
                _this->trace.msg("[Redmule] Set M size 0x%x)\n", value & 0xFFFF);
                _this->k_size = (value >> 16) & 0xFFFF;
                _this->trace.msg("[Redmule] Set K size 0x%x)\n", (value >> 16) & 0xFFFF);
            case 80:
                _this->n_size = value;
                _this->trace.msg("[Redmule] Set N addr 0x%x)\n", value);
                break;
            case 84:
                _this->trace.msg("[Redmule] Matrix Arithmetic Reg\n");
                break;
            default:
                _this->trace.msg("[Redmule] write to INVALID address\n");
        }
    }
...
vp::IoReqStatus Redmule::send_tcdm_req()
{
    ...
    switch (_this->state.get()) {
        ...
        case PRELOAD:
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Preload] --- Send TCDM req #%d\n",_this->fsm_counter);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[Redmule][Preload] There was an error while reading/writing data\n");
                    return;
                }
            ...
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp) ){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Preload] ---                           Receive TCDM resp\n");
        ...
        case ROUTINE:
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                //Send request
                vp::IoReqStatus err = _this->send_tcdm_req();
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][ROUTINE-ijk: %0d-%0d-%0d] --- Send TCDM req #%d\n", _this->iter_i, _this->iter_j, _this->iter_k, _this->fsm_counter);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[Redmule][ROUTINE-ijk: %0d-%0d-%0d] There was an error while reading/writing data\n", _this->iter_i, _this->iter_j, _this->iter_k);
                    return;
                }
            ...
            //Recieve Process
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp) ){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][ROUTINE-ijk: %0d-%0d-%0d] ---                           Receive TCDM resp\n", _this->iter_i, _this->iter_j, _this->iter_k);
                _this->pending_req_queue.pop();
            }
            ...
            if ((_this->fsm_counter >= _this->tcdm_block_total) && (_this->pending_req_queue.size() == 0))
            {
                ...
                {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][ROUTINE-ijk: %0d-%0d-%0d] TCDM Access Time(%d) > Model Time(%d)\n", _this->iter_i, _this->iter_j, _this->iter_k, _this->fsm_timestamp, modeled_runtime);
                    latency = 1;
                } else {
                    _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][ROUTINE-ijk: %0d-%0d-%0d] Model Time(%d) > TCDM Access Time(%d)\n",  _this->iter_i, _this->iter_j, _this->iter_k, modeled_runtime, _this->fsm_timestamp);
        ...
        case STORING:
            ...
            if ((_this->fsm_counter < _this->tcdm_block_total) && (_this->pending_req_queue.size() <= _this->queue_depth))
            {
                ...
                //Send request
                vp::IoReqStatus err = _this->send_tcdm_req();
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Storing] --- Send TCDM req #%d\n",_this->fsm_counter);

                //Check error
                if (err != vp::IO_REQ_OK) {
                    _this->trace.fatal("[Redmule][Storing] There was an error while reading/writing data\n");
                    return;
                }
            ...
            //Recieve Process
            while((_this->pending_req_queue.size()!= 0) && (_this->pending_req_queue.front() <= _this->fsm_timestamp) ){
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][Storing] ---                           Receive TCDM resp\n");
                _this->pending_req_queue.pop();
            }
            ...
            if ((_this->fsm_counter >= _this->tcdm_block_total) && (_this->pending_req_queue.size() == 0))
            {
                ...
                _this->num_matmul      += 1;
                _this->trace.msg("[Redmule] Finished : %0d ns ---> %0d ns | period = %0d ns (%0d cyc) | uti = %0.3f | runtime = %0d ns | GEMM id = %0d\n", start_time_ns, end_time_ns, period_ns, period_clk, period_uti, _this->total_runtime, _this->num_matmul);
            } else {
                _this->state.set(STORING);
                _this->event_enqueue(_this->fsm_event, 1);
            }
        ...
        case FINISHED:
            ...
            {
                _this->state.set(FINISHED);
                _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][FINISHED] Waiting for query!\n");
            } else {
        ...
        case ACKNOWLEDGE:
            ...
            _this->trace.msg(vp::Trace::LEVEL_TRACE,"[Redmule][ACKNOWLEDGE] Done!\n");
            break;

        default:
            _this->trace.fatal("[Redmule] INVALID RedMule Status: %d\n", _this->state);
    }
}
